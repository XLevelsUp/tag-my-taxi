...
const moreFeatures = [
  { icon: <Wallet className="w-8 h-8 text-[#E31E24]" />, title: 'Wallet payment', desc: 'Host wallet option for both drivers and passengers for easier checkout and secure payment.' },
  { icon: <PhoneCall className="w-8 h-8 text-[#E31E24]" />, title: 'Call masking', desc: 'Protect private information on calls and texts by masking the permanent number of the users.' },
  { icon: <Mic className="w-8 h-8 text-[#E31E24]" />, title: 'Voice commands', desc: 'Integrate VoIP in the dispatcher panel to stay in touch with your resources at no additional costs.' },
  { icon: <Heart className="w-8 h-8 text-[#E31E24]" />, title: 'Loyalty program', desc: 'Avail discounts and rewards to persuade customers to engage more frequently with your services.' },
  { icon: <Share2 className="w-8 h-8 text-[#E31E24]" />, title: 'Social sharing', desc: 'Link relevant social media sites to bring on supporting reviews and feedback on your services.' },
  { icon: <Languages className="w-8 h-8 text-[#E31E24]" />, title: 'Multi-lingual support', desc: 'Incorporate multiple languages in the application to reach more customers at the global level.' },
]
    ...
    <ul className="space-y-8">
      {features.map((f, i) => (
        <li key={i} className="flex gap-5 items-start">
          <span className="flex-shrink-0 w-9 h-9 rounded-full border-2 border-[#E31E24] text-[#E31E24] font-bold text-sm flex items-center justify-center">
            {i + 1}
          </span>
          <div>
    ...
    <div className="flex flex-col min-h-screen bg-white font-sans text-gray-900">
      ...
      <header className="pt-40 pb-16 bg-gray-50 border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-6 text-center space-y-3">
          <span className="text-[#E31E24] font-bold uppercase tracking-widest text-xs">Platform Overview</span>
